    ...
    fflush(stdin);
	   ...
	   do{
	    ...
	    if(equipaje==1){
	    	...
	    	scanf("%d",&bultos);
	    		if(bultos>3){
	    		printf("Le recordamos que no es posible facturar mas de 3 bultos por persona\n");
			} 
	    } while(bultos>3);
...
for(j=0;j<nbilletes;j++){
	...
	printf("\nIntroduzca el nombre del pasajero %d: ",j+1);
 	...
 	scanf("%s",pasajero[j].edad); //Si ponemos %d para la edad no se guarda bien en el fichero
 	if(pasajero[j].edad>0 && pasajero[j].edad<3){ 
 		printf("Se aplica la tarifa Baby\n");
	} else if(pasajero[j].edad>3&&pasajero[j].edad<12){
		printf("Se aplica la tarifa Children\n");
	} else if (pasajero[j].edad>12 &&pasajero[j].edad<65){
		printf("Se aplica la tarifa Adult\n");
	} else{
		printf("Se aplica la tarifa Senior\n");
	}
